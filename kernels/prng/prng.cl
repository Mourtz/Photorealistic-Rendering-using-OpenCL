...
#if RNG_TYPE == 0
inline float next1D(RNG_SEED_PARAM) {
	/* hash the seeds */
...
#define nextBoolean(c, RNG_SEED_VALUE) (next1D(RNG_SEED_VALUE) < c)
#define next2D(RNG_SEED_VALUE) (float2)(next1D(RNG_SEED_VALUE), next1D(RNG_SEED_VALUE))
