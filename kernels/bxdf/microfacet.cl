...
float Microfacet_D(int dist, float alpha, const float3 m)
{
    ...
    if (dist & BECKMANN) {
        ...
        return INV_PI * exp(-tanThetaSq / alphaSq) / (alphaSq * cosThetaQu);
    }
    else if (dist & PHONG) {
...
float Microfacet_G1(int dist, float alpha, const float3 v, const float3 m)
{
    ...
    if (dist & BECKMANN) {
        ...
        float tanTheta = fabs(sqrt(fmax(1.0f - cosThetaSq, 0.0f)) / v.z);
...
float3 Microfacet_sample(int dist, float alpha, float2 xi)
{
    ...
    if (dist & BECKMANN) {
        float tanThetaSq = -alpha * alpha * log(1.0f - xi.x);
