...
void LambertianFiberBCSDF(
	...
	float h = get_random(RNG_SEED_NAME)*2.0f - 1.0f;
	float nx = h;
    float nz = trigInverse(nx);

	float2 xi = hash_2ui_2f32(RNG_SEED_NAME);
...
#define wi event->wi
	...
	float3 m = Microfacet_sample(dist, sampleAlpha, hash_2ui_2f32(RNG_SEED_NAME));
	...
	bool reflect = get_random(RNG_SEED_NAME) < F;
...
#define wi event->wi
	...
	float3 m = Microfacet_sample(dist, alpha, hash_2ui_2f32(RNG_SEED_NAME));
	float wiDotM = dot(wi, m);
