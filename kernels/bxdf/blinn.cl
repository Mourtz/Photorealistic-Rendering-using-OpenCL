...
float3 SampleBlinn(float3 n, float alpha, RNG_SEED_TYPE) {
	float phi = TWO_PI * get_random(RNG_SEED_NAME);
	float cosTheta = pow(get_random(RNG_SEED_NAME), 1.0f / (alpha + 1.0f));
