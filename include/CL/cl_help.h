...
static void printErrorLog(const cl::Program &program, const cl::Device &device)
{
...
{
template <typename T>
static inline cl::Buffer create(
	std::vector<T> data,
	std::size_t size,
...
namespace platform
{
static void select(cl::Platform &platform, const std::vector<cl::Platform> &platforms)
{
...
namespace device
{
static void select(cl::Device &device, const std::vector<cl::Device> &devices)
{
...
static std::string parse(std::string filepath, host_scene *scene)
{
	using std::string;
...
static cl::Program LoadProgram(std::string filepath, std::string opts, std::vector<cl::Device> devices)
{
